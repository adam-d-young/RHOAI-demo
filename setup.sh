...
if [ -n "${RHOAI_USER:-}" ]; then
  echo "    Username: ${RHOAI_USER}"
  echo "    Password: ${RHOAI_PASS}"
else
  echo "    Username: (htpasswd user created in a previous run)"
  echo "    Password: (set during previous run -- check your notes)"
fi
echo ""
echo "  MinIO:"
echo "    URL:      https://$(oc get route minio-ui -o jsonpath='{.spec.host}' 2>/dev/null || echo '(not available)')"
echo "    Username: minio"
